...
export default function Layout({
    ...
    useEffect(() => {
        if (!loading) {
            ...
            } else if (profile?.ranks !== 'Principal') {
    ...
    if (!profile || profile.ranks !== 'Principal') return null; // Prevent flash
                ...
                <header className="flex h-16 shrink-0 items-center gap-2 border-b border-white/5 bg-black/20 backdrop-blur-md px-4 transition-[width,height] ease-linear group-has-[[data-collapsible=icon]]/sidebar-wrapper:h-12">
                        ...
                        <Breadcrumb>
                            <BreadcrumbList className="text-[10px] uppercase font-black tracking-widest text-zinc-500">
                                <BreadcrumbItem className="hidden md:block">
                                    <BreadcrumbLink href="/PrincipalDashboard" className="hover:text-emerald-500">
                                        Admin
                                    </BreadcrumbLink>
                                </BreadcrumbItem>
                                <BreadcrumbSeparator className="hidden md:block opacity-20" />
                                <BreadcrumbItem>
                                    <BreadcrumbPage className="text-zinc-200">Global Overview</BreadcrumbPage>
                                </BreadcrumbItem>
                            </BreadcrumbList>
                        </Breadcrumb>
                    </div>
                    <div className="ml-auto flex items-center gap-4">
                        <div className="flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-500/10 border border-emerald-500/20">
                            ...
                            <span className="text-[10px] font-bold text-emerald-500 uppercase tracking-tighter">Systems Live</span>
                        </div>
                    </div>
                </header>
