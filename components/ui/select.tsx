...
function SelectContent({
  ...
  >) {
  // Extract z-index from className if present, otherwise use default z-50
  const zIndexMatch = typeof className === 'string' ? className.match(/z-\[(\d+)\]|z-(\d+)/) : null
  const zIndexClass = zIndexMatch ? (zIndexMatch[0] as string) : "z-50"
  ...
  return (
    <SelectPrimitive.Portal>
      <SelectPrimitive.Positioner
        ...
        className={cn("isolate", zIndexClass)}
      >
